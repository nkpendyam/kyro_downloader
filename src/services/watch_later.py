...
def get_watch_later_url(playlist_id=WATCH_LATER_PLAYLIST_ID):
    return WATCH_LATER_URL_TEMPLATE.format(playlist_id)

def get_watch_later_info():
...
def is_watch_later_url(url):
    playlist_id = extract_playlist_id(url)
    return playlist_id == WATCH_LATER_PLAYLIST_ID or "watchlater" in url.lower()

def build_watch_later_download_config(cookies_file=None):
