...
def _format_platform_label(platform: dict) -> str:
    icon = str(platform.get("icon", "")).strip()
    name = str(platform.get("name", "")).strip() or "Unknown"
...
def create_parser():
    parser = argparse.ArgumentParser(
        prog="kyro",
        description="Kyro Downloader - Production-grade media downloader by nkpendyam",
    ...
    dl.add_argument(
        ...
        type=str,
        choices=["chrome", "firefox", "edge", "brave", "opera", "safari"],
        help="Load cookies directly from browser profile",
    )
    ...
    dl.add_argument(
        "--preset",
        choices=["none", "voice-optimized", "music-lossless", "podcast-fast"],
        default="none",
        help="Apply competitor-grade media preset",
    )
    dl.add_argument("--dry-run", action="store_true", help="Show selected format and exit without downloading")
    ...
    mp3.add_argument(
        "--prefer-codec",
        default="any",
        choices=["any", "opus", "aac", "mp3", "flac", "alac", "wav", "ogg"],
        help="Preferred codec when --smart-audio is enabled",
    )
    ...
    mp3.add_argument("--dry-run", action="store_true", help="Show effective audio settings and exit")
    ...
    pl.add_argument("--dry-run", action="store_true", help="Show playlist plan and exit")
...
def cmd_download(args, config):
    ...
    if _is_dry_run(args):
        print(f"\n[bold yellow]Dry run:[/bold yellow] would download format {format_id} to {output}")
        return
...
def cmd_mp3(args, config):
    ...
    if _is_dry_run(args):
        print(
            f"\n[bold yellow]Dry run:[/bold yellow] would download audio ({cfg['audio_format']} @ {cfg['audio_quality']}k) to {output}"
        )
        return
    manager.config.update(cfg)
...
def cmd_playlist(args, config):
    ...
    if getattr(args, "cookies_from_browser", None):
        cfg["cookies_from_browser"] = args.cookies_from_browser
    pl_cfg = {
        "concurrent_downloads": args.workers,
        "sleep_interval": args.sleep,
        "max_downloads": args.max,
        "playlist_reverse": args.reverse,
        "playlist_random": args.random,
    }
    if args.mp3:
    ...
    if preset:
        print(f"[bold cyan]Preset applied:[/bold cyan] {args.preset}")
    if _is_dry_run(args):
        print(f"\n[bold yellow]Dry run:[/bold yellow] would download playlist to {output}")
        return
    manager.config.update(cfg)
...
def cmd_batch(args, config):
    ...
    if _is_dry_run(args):
        print(
            f"\n[bold yellow]Dry run:[/bold yellow] would queue {len(urls)} URL(s) to {output} with {args.workers} workers"
        )
        return
    manager.config.update(cfg)
    if preset:
...
def cmd_info(args, config):
    ...
    if info.entries:
        print(f"[bold]Playlist Items:[/bold] {len(info.entries)}")
    platform = get_platform_info(url)
    if platform:
        print(f"[bold]Platform:[/bold] {_format_platform_label(platform)}")
    if hasattr(config, "ui") and getattr(config.ui, "show_thumbnail", False):
        show_thumbnail_inline(info.thumbnail)
    if args.subs:
...
def cmd_platforms(args, config):
    ...
    for p in platforms:
        table.add_row(
            ...
            "Yes" if p.get("supports_hdr") else "No",
            "Yes" if p.get("supports_dolby") else "No",
            p.get("max_resolution", "?"),
        )
    console.print(table)
...
def main():
    ...
    if args.update:
        if update_ytdlp():
            print("[bold green]yt-dlp updated![/bold green]")
        else:
            print("[bold red]Update failed[/bold red]")
        return
    if not args.command:
